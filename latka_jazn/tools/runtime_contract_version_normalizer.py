...
SCHEMA_PREFIXES = {
    "manifest_current": lambda version: f"manifest_current/v{_version_number(version)}",
...
def normalize_runtime_contract_versions(root: Path, *, apply: bool = False) -> dict[str, Any]:
    ...
    manifest_sha = _sha256_file(root / "MANIFEST_CURRENT.json")
    ...
    if apply and manifest_sha and marker_path.exists():
        ...
        old = marker.get("manifest_current_sha256")
        if old != manifest_sha:
            ...
            marker_sha_update = {"path": f"{marker_rel}.manifest_current_sha256", "old": old, "new": manifest_sha}
