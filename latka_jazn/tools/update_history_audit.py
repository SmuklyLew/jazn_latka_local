...
def write_index_json(root: Path) -> Path:
    root=Path(root); out=root/'docs'/'update_history'/'INDEX.json'; out.parent.mkdir(parents=True, exist_ok=True)
    entries=[]
    for p in collect_manifest_files(root):
        ...
        if 'declared_missing_code' in statuses: audit='partial'
        elif any(s in statuses for s in ['implemented','implemented_no_test']): audit='ok' if 'implemented_no_test' not in statuses else 'partial'
        else: audit='unchecked'
        entries.append({'path':p.relative_to(root).as_posix(),'filename':p.name,'declared_version':data.get('version') or data.get('runtime_version') or data.get('target_version'), 'filename_version_hint':p.stem, 'schema_version':data.get('schema_version'), 'kind':'update_manifest', 'status':'historical', 'parse_ok':ok, 'parse_error':err, 'sha256':_sha(p), 'declared_features':features, 'declared_files':files, 'declared_tests':tests, 'implementation_audit_status':audit, 'notes':[]})
    payload={'schema_version':SCHEMA_VERSION,'generated_at_utc':datetime.now(timezone.utc).isoformat(),'active_version':(root/'VERSION.txt').read_text(encoding='utf-8').strip() if (root/'VERSION.txt').exists() else 'unknown','entries':entries,'entry_count':len(entries),'truth_boundary':'Indeks porządkuje historyczne manifesty. Nie oznacza, że wszystkie stare deklaracje muszą być aktywną funkcją runtime.'}
