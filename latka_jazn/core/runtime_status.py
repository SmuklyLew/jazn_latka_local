...
def build_runtime_status(config: JaznConfig | None = None, store: Any | None = None, *, readonly: bool = False) -> str:
    ...
    event_ledger_counts = {}
    for name in ["runtime_events.jsonl", "conversation_turns.jsonl", "runtime_event_errors.jsonl"]:
        path = raw / name
        if path.exists():
            try:
                event_ledger_counts[name] = sum(1 for line in path.open("r", encoding="utf-8") if line.strip())
            except Exception:
                event_ledger_counts[name] = "error"
        else:
            event_ledger_counts[name] = "missing"
    ...
    state_note = "ciągłość przerw jest zapisywana w workspace_runtime/runtime_state.json, ale to nadal nie jest demon w tle"
