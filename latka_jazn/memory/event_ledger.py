...
class RuntimeEventLedger:
    ...
    Rozdzielenie:
    - `memory/raw/runtime_events.jsonl` — pełny surowy log zdarzeń runtime;
    - `memory/raw/conversation_turns.jsonl` — pełne tury rozmowy user/assistant;
    - `memory/raw/runtime_event_errors.jsonl` — awaryjny log błędów zapisu.
    ...
    def __init__(self, root: Path, *, version: str, timezone_name: str = DEFAULT_TIMEZONE) -> None:
        ...
        self.runtime_events_path = self.root / "memory" / "raw" / "runtime_events.jsonl"
        self.conversation_turns_path = self.root / "memory" / "raw" / "conversation_turns.jsonl"
        self.errors_path = self.root / "memory" / "raw" / "runtime_event_errors.jsonl"
    ...
    def append_event(
        ...
        *,
        ...
        try:
            ...
            return self._append_jsonl(self.runtime_events_path, record)
