...
def parse_semver(label: str) -> tuple[int, int, int]:
    ...
    if not match:
        raise ValueError(f"Nie można odczytać numeru semver z wersji: {label!r}")
    return tuple(int(part) for part in match.groups())  # type: ignore[return-value]


def next_patch_version(current_label: str, *, suffix: str = DEFAULT_SUFFIX) -> str:
    major, minor, patch = parse_semver(current_label)
...
def read_current_version(root: Path) -> str:
    ...
    raise FileNotFoundError("Brak VERSION.txt i nie udało się odczytać wersji z pyproject.toml")


def version_number_only(version_label: str) -> str:
    major, minor, patch = parse_semver(version_label)
    return f"{major}.{minor}.{patch}"


def update_version_files(root: Path, target_version: str, description: str) -> None:
    (root / "VERSION.txt").write_text(target_version.strip() + "\n", encoding="utf-8")
    pyproject = root / "pyproject.toml"
    if pyproject.exists():
        text = pyproject.read_text(encoding="utf-8")
        text = re.sub(r'^version\s*=\s*"[^"]+"', f'version = "{version_number_only(target_version)}"', text, flags=re.MULTILINE)
        text = re.sub(r'^description\s*=\s*"[^"]*"', f'description = {json.dumps(description, ensure_ascii=False)}', text, flags=re.MULTILINE)
...
def append_readme_section(root: Path, *, target_version: str, doc_rel: str) -> None:
    ...
    if not readme.exists():
        return
    marker = f"## {target_version}"
    text = readme.read_text(encoding="utf-8")
    if marker in text:
        return
    section = f"""

## {target_version} — Conversation Memory Capture
...
def write_update_protocol(root: Path) -> None:
    protocol = {
        ...
        "default_behavior": {
            "mode": "memory-only",
            "versioning": "odczytaj VERSION.txt i automatycznie podnieś patch o 1",
...
def run_auto_memory_update(
    *,
    ...
    memories = memories or [
        ...
        "Numer wersji ma być wyliczany automatycznie z VERSION.txt.",
