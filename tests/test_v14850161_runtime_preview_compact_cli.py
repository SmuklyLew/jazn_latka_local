...
class _FakeEngine:
    def __init__(self, config=None) -> None:
        self.config = SimpleNamespace(version="v14.8.5.016.1", root=Path("/tmp/fake-jazn-root"))
...
def test_runtime_preview_stdout_is_compact_not_full_cognitive_dump(monkeypatch, capsys) -> None:
    ...
    assert payload["schema_version"] == "runtime_preview_compact/v14.8.5.016.1"
...
def test_runtime_preview_output_writes_full_payload_but_keeps_stdout_compact(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    assert stdout_payload["schema_version"] == "runtime_preview_compact/v14.8.5.016.1"
    ...
    assert full_payload["schema_version"] == "runtime_preview_full_payload/v14.8.5.016.1"
...
def test_dev_preview_is_explicit_full_payload_mode(monkeypatch, capsys) -> None:
    ...
    assert payload["schema_version"] == "runtime_preview_full_payload/v14.8.5.016.1"
