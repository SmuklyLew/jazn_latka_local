...
def test_runtime_preview_stdout_is_compact_not_full_cognitive_dump(monkeypatch, capsys) -> None:
    ...
    assert payload["schema_version"] == f"runtime_preview_compact/{PACKAGE_VERSION}"
...
def test_runtime_preview_output_writes_full_payload_but_keeps_stdout_compact(monkeypatch, capsys, tmp_path: Path) -> None:
    ...
    assert stdout_payload["schema_version"] == f"runtime_preview_compact/{PACKAGE_VERSION}"
    ...
    assert full_payload["schema_version"] == f"runtime_preview_full_payload/{PACKAGE_VERSION}"
...
def test_dev_preview_is_explicit_full_payload_mode(monkeypatch, capsys) -> None:
    ...
    assert payload["schema_version"] == f"runtime_preview_full_payload/{PACKAGE_VERSION}"
